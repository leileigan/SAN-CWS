...
class CRF(nn.Module):
    ...
    def _calculate_PZ(self, feats, mask):
        ...
        cur_values = self.transitions.view(1, tag_size, tag_size).expand(batch_size, tag_size, tag_size) + partition.contiguous().view(batch_size,
                                                                                                                                       tag_size,
                                                                                                                                       1).expand(
    ...
    def _viterbi_decode(self, feats, mask):
        ...
        partition_history = torch.cat(partition_history).view(seq_len, batch_size, -1).transpose(1,
    ...
    '''
    def decode(self, feats, mask):  # Viterbi decoding
        ...
        for t in range(feats.size(1)):  # recursion through the sequence
            ...
            score_t, bptr_t = score_t.max(2)  # best previous scores [B, C] and tags [B, C]
            score_t += feats[:, t]  # plus emission scores [B, C]
...
'''
crf = CRF(5, False)
feats = torch.randn(10, 2, 7)
mask = torch.zeros(10, 2).long()
crf._viterbi_decode(feats, mask)
'''
